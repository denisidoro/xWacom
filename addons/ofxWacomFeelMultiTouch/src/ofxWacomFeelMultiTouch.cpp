...
bool ofxWacomFeelMultiTouch::init(bool observer)
{
	...
	if(WacomMTInitialize != NULL)
	{
		...
		if(err == WMTErrorSuccess)
		{
			...
			mode = observer ? WMTProcessingModeObserver : WMTProcessingModeNone;
			...
			else
			{
				...
				{
					WacomMTRegisterFingerReadCallback(deviceIDs[counter], NULL, mode, ofxWacomFeelMultiTouchFingerCallback, this);
				}
	...
	coordinateWidth		= 1.0f; //ofGetWidth();
	coordinateHeight	= 1.0f; //ofGetHeight();	
